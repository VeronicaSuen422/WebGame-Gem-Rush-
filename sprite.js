...
const Sprite = function(ctx, x, y) {
    ...
    const drawSprite = function() {
        ...
        // ctx.clearRect(parseInt(x-size.width/2), parseInt(y-size.height/2), size.width, size.height);
        ctx.imageSmoothingEnabled = false;
        ctx.drawImage(sheet, sequence.x + index * sequence.width, sequence.y, sequence.width, sequence.height, parseInt(x-size.width/2), parseInt(y-size.height/2), size.width, size.height);
